...
- [`DeviceTagging.sanitized.json`](DeviceTagging.sanitized.json) -- Logic App definition. Tenant, subscription, RG, connections, and managed identity references are placeholders.
...
1. Open `DeviceTagging.sanitized.json`. Search-replace placeholders for tenant ID, subscription ID, resource group, Logic App name, connection IDs, and managed identity references.
